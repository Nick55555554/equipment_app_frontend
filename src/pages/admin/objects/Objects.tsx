...
const AdObjects = () => {
    ...
    const toExcel = (id: number) => {
        const token = document.cookie.split('=')[1];
        const url1 = `${url}/excel/named_equipment/${id}`;
            ...
            <tbody>
                {objects.map((object) => (
                    <tr key ={object.id} >
                        ...
                        <td className="td">
                            {object.requestsSent}<img src={excel} alt="Подробнее" style={{marginLeft:"150px", height: "32px", width: "32px"}} onClick={() => toExcel(object.id)}/>
                        </td>
                    </tr>
                ))}
            </tbody>
            ...
            className="table">
                    <thead>
                        <tr>
                            ...
                            <th className='left_th'>Заявок принято</th>
                        </tr>
                    </thead>
                    {isPending ? 
                    (<tbody>

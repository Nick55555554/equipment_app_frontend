...
import { useDisTechnicRouter } from "../../../router/PageCreators/DisTechnicCreator";
...
const TechnicsDis = () => {
    ...
    const {createDisTechnicRoute} = useDisTechnicRouter();
    ...
    useEffect(() => {
        if (technic) {
            createDisTechnicRoute(technic);
            localStorage.setItem(location.pathname,location.pathname)
            navigate(`/dispatcher/markedtechnic${technic}`);
        }
                ...
                technic.equipmentTypeResponses.length > 0 ? (
                    technic.equipmentTypeResponses.map((one) => (
                        ...
                        <td className="left_td last_t">
                            <div className="statusDis">{one.count}</div>
    ...
    const filteredTechnics: TechnicTypes[] = filter
